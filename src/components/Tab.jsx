...
export default function Tab() {
    ...
    const tabs = [
        ...
        {
            id: 5,
            icon: <View className='flex items-center justify-center h-[30px] w-[30px] bg-red-500 rounded-full'>
  ...
  return (
    <View className='flex flex-col items-center justify-start w-full h-full'>
          {count === 1 ? <Home /> : count === 2 ? <Cards /> 
          : count === 3 ? <SendMoney /> : count === 4 ? <Bitcoins /> 
          : count === 5 && <Profile />}
         <View className='absolute bottom-0 flex flex-row items-end justify-between w-full h-[80px] bg-white shadow-2xl border-[0.8px] border-[#eeeeee] px-5 pb-4'>
              {tabs.map((item) => {
                return(
                    <TouchableOpacity onPress={()=>handleTab(item.id)}
                    key={item.id} className='flex flex-col items-center justify-center'>
                              <View className={`flex items-center justify-center bg-[${item.id === count && '#CDEAFC'}] rounded-full h-10 w-10`}>
                                   {item.icon}
                              </View>
                              <Text className={`text-[9px] text-[#029CFC] font-["sans-medium"]`}>{item.label}</Text>
                    </TouchableOpacity>
                )
              })}
         </View>
