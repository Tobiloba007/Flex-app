...
export default function Verification({ isDark }) {
  ...
  return (
    ...
    >
      ...
      <View className="items-start w-full px-5 mt-9">
        ...
        <Text
          className={`text-13px] font-["sans-regular"] pt-1`}
          style={{ color: isDark ? colors.white : colors.black }}
        >
          Enter the security code we just sent your email to confirm your email
          address.
        </Text>
      </View>
      ...
      <View className="items-center w-full px-5 mt-11">
        ...
        <View className="flex items-start w-full mt-10">
          {errorMssg && (
            <Text className="text-sm text-red-600">{errorMssg}</Text>
          )}
        </View>
        ...
        <Pressable onPress={() => navigation.navigate("resendCode")}>
          <Text
            className={`text-[11px] font-["sans-regular"] text-[#029CFC] mt-3`}
