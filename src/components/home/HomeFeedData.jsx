...
const HomeFeedData = ({ item }) => {
  ...
  const [isLiked, setIsLiked] = useState(
    item?.likes_users?.includes(userDet?.id) ? true : false
  );
  ...
  const handleLike = async () => {
    ...
    if (isLiked) {
      ...
      try {
        const res = await axios.put(`${BASE_URL2}/post/${item.post_id}/like`, {
    ...
    <View className="flex flex-col items-start justify-start w-full mt-5">
      <View className={"flex flex-row items-start justify-start w-full"}>
        ...
        <View className={"flex flex-col items-start justify-start w-[17%]"}>
          ...
          >
            <Image className="w-5 h-5 rounded-full" source={item?.replyImg1} />
            <Image
        ...
        >
          ...
          <View className={"flex flex-row items-center justify-between w-full"}>
            <View className={"flex flex-row items-center justify-start"}>
              ...
              {userDet?.verify && (
                <MaterialIcons name={"verified"} size={18} color="#029CFC" />
              )}
            </View>
