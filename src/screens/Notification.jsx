...
export default function Notification() {
    ...
    const notifications = [
        {
            id: 1,
            icon: <MaterialCommunityIcons name="notebook-multiple" size={22} color="#029CFC" />,
        ...
        {
            id: 2,
            icon: <FontAwesome6 name="arrow-down-wide-short" size={20} color="#029CFC" />,
  ...
  return (
    <View className='flex flex-col items-center justify-start w-full h-full bg-white' style={{paddingTop: StatusBar.currentHeight}}>
         <View className='flex items-center justify-center w-full h-[80px] bg-white shadow-3xl border-b-[1px] border-[#eeeeee]'>
             <Text className={`text-center text-black font-["sans-semibold"] text-lg mt-4`}>Notifications</Text>
         ...
         <View className='flex flex-col items-center justify-start w-full px-5 mt-1'>
               {notifications.map((item)=>{
                return(
                    <View key={item.id} className='flex flex-col items-center justify-start w-full mt-3'>
                          <View className='flex flex-row items-center justify-between w-full'>
                                 <View className='flex flex-row items-center justify-start'>
                                     {item.icon}
                                    <Text className={`text-center text-black font-["sans-bold"] text-sm ml-2`}>{item.title}</Text>
                                </View>
                                <Text className={`text-center text-black font-["sans-bold"] text-sm ml-2`}>{item.time}</Text>
                          </View>
                         <Text className={`text-left w-full text-gray-500 font-["sans-semibold"] text-[11px] mt-1`}>
                               {item.desc}
                         </Text>
